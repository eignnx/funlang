...
import Control.Monad.State (MonadTrans (lift), evalStateT, foldM)
import qualified Data.Map as M
import Data.Maybe (fromMaybe)
import Data.Semigroup
import Debug.Trace (trace, traceM)
import qualified Intr
import Res (Error (RootCause), Res, ensureM, toRes)
import Tcx
  ( NoAliasTy (..),
    TyChecker,
    define,
...
instance CheckType Ast.RefutPat where
  ...
  check pat@(Ast.TupleRefutPat ps) (TupleTy ts)
    ...
    | otherwise = fail $ "The pattern" +++ code pat +++ "can't be bound to a tuple that has" +++ show (length ts) +++ "elements"
  check (Ast.TupleRefutPat _) _ = undefined
...
checkArgs fn args argTys
  ...
  where
    msg =
      "Oops! You passed" +++ show received +++ "arguments to"
        +++ code fn ++ ", but it expects"
        +++ show expected
...
itemName expr = Ast.itemName expr `toRes` RootCause msg
  where
    msg = "The expression" +++ code expr +++ "can't appear in the top level"
...
instance CheckType Ast.Expr where
          ...
          <$> forA
            ...
            ( \case
                ...
                Ast.TyDefF Ast.NotRec tyName defs :@: loc -> do
                  let f (Ast.VrntDef name tys) = return $ M.singleton name (map getNonRecTy tys)
                        where
                          getNonRecTy = \case
                            Ast.NonRecTy ty -> ty
                ...
                Ast.TyDefF Ast.IsRec tyName defs :@: loc -> do
                  ...
                  let f (Ast.VrntDef name tys) = return $ M.singleton name (map g tys)
                        where
                          g = \case
                            Ast.NonRecTy ty -> ty
  ...
  check (Ast.MatchF scrut arms :@: loc) ty = do
    ...
    let reducer :: Maybe Ty -> Ty -> TyChecker (Maybe Ty)
