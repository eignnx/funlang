...
import qualified Ty
import Utils (braces, code, codeIdent, optList, (+++))
...
defineFn name compilation = do
  ...
  case maybeLbl of
    Just lbl -> do
      ...
      return lbl
    Nothing -> error $ "Internal Compiler Error: unknown fn name" +++ codeIdent name
...
instance Compile (Ast.Seq Ast.TypedExpr) where
  ...
  compile (Ast.Semi expr@(exprF :<: NoAliasPat ty) seq)
    | isZeroSized ty = do
      -- Already Void type, no need to Pop.
      expr' <- compile expr
      seq' <- compile seq
      return $ expr' ++ seq'
    | otherwise = do
      -- This one needs to discard its non-Void result.
      expr' <- compile expr
      seq' <- compile seq
...
instance Compile (Ast.RefutPat, Hir.Lbl, NoAliasTy) where
  compile = \case
    ...
    (pat@(Ast.VrntRefutPat name params), next, NoAliasPat (Ty.VrntTy vrnts)) -> do
      ...
      let testDiscr =
            [ Hir.Dup :# "Need a copy for discriminant test",
...
instance Compile Ast.TypedExpr where
  ...
  compile (Ast.TyDefF _ name ctorDefs :<: ty) = do
    return []
  compile (Ast.LetConstF {} :<: _) = undefined
